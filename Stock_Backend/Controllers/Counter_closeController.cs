...
namespace Stock_Backend.Controllers
{
    public class Counter_closeController : ApiController
    {
        ...
        [HttpGet]
        public HttpResponseMessage GetCounterData(int Counter_id)
        {
            try
            {
                db.Connect();

                // Counter Info - most recent open counter
                string counterQuery = @"
SELECT TOP 1
    ac.Id,
    ac.Counter_id,
...
INNER JOIN VIEW_EMPLOYEE_INFO e ON e.Emp_id = ac.Emp_id
...
ORDER BY ac.Id DESC";
                ...
                {
                    db.Disconnect();
                    return Request.CreateResponse(HttpStatusCode.BadRequest, "Counter not found or already closed");
                }

                var row = dt.Rows[0];

                // Sales - Counter wise, Login_date from today
                string salesQuery = @"
SELECT
    ISNULL(MIN(s.Pavati_no), 0)                                 AS Bill_from,
    ISNULL(MAX(s.Pavati_no), 0)                                 AS Bill_to,
    ISNULL(SUM(s.Final_amt), 0)                                 AS Total_sale,
    ISNULL(SUM(s.Final_amt - s.UPI_amt - s.Redeem_amt), 0)     AS Cash_sale,
...
AND CAST(s.Sale_date AS DATE) = @LoginDate";
                ...
                cmdSales.Parameters.AddWithValue("@LoginDate", Convert.ToDateTime(row["Login_date"]).Date);
        ...
        [Route("api/CounterClose/GetClosedData")]
        [HttpGet]
        public HttpResponseMessage GetClosedCounterData(int Counter_id)
        {
            try
            {
                db.Connect();
                ...
                {
                    db.Disconnect();
                    return Request.CreateResponse(HttpStatusCode.BadRequest, "No closed counter data found");
                }
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new
                {
                    ...
                    Office_return = row["Office_return"],
                    Logout_date = row["Log_out_date"],
                    Logout_time = row["Log_out_time"]
                });
            }
            catch (Exception ex)
            {
